...
import { formatEnergy, formatDateTime, timeAgo } from '../../utils/format';
...
export const Home = () => {
  ...
  const doorLocks = safeDevices.filter(d => d.deviceType === 'door-lock' || d.type === 'door_lock');
  const lights = safeDevices.filter(d => d.deviceType === 'light' || d.type === 'light');
  const plugs = safeDevices.filter(d => d.deviceType === 'plug' || d.type === 'plug');
  const motionSensors = safeDevices.filter(d => d.deviceType === 'motion' || d.type === 'motion');
  ...
  // Use dashboard stats if available, fallback to local calculation
  const statsData = dashboardStats?.devices || {
    total: safeDevices.length,
    online: onlineDevices.length,
    offline: safeDevices.length - onlineDevices.length,
  ...
  const stats = [
    ...
    },
    {
      ...
      value: dashboardStats?.visitors?.pending || 0,
      icon: (
                    ...
                    <div key={activity.id} className="p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex items-start space-x-3">
                        <div className="flex-shrink-0">
                          ...
                          {!['door', 'motion'].includes(activity.icon) && (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-gray-500">{activity.deviceName}</p>
                          {activity.username && (
